    ...
    if (!rst_n) begin
      ...
      case (tx_state)
        ...
        TX_SEND: begin
          if (!tx_busy) begin
            case (tx_byte_count)
              2'b00: tx_byte <= tx_data_reg[7:0];
              2'b01: tx_byte <= tx_data_reg[15:8];
              2'b10: tx_byte <= tx_data_reg[23:16];
              2'b11: tx_byte <= tx_data_reg[31:24];
            endcase
            tx_valid <= 1'b1;
    ...
    if (!rst_n) begin
      ...
      case (rx_state)
        RX_RECV: begin
          if (rx_done) begin
            case (rx_byte_count)
              2'b00: rx_data_reg[7:0] <= rx_byte;
              2'b01: rx_data_reg[15:8] <= rx_byte;
              2'b10: rx_data_reg[23:16] <= rx_byte;
              2'b11: rx_data_reg[31:24] <= rx_byte;
            endcase
